...
- **Expected Output:**
  ...
  - **Owner audit:** Duyệt toàn bộ 24 trang `/owner/*` bằng tài khoản `courtowner@prosport.vn` trên browser thật; sửa build backend bị chặn (`EscrowServiceTests` thiếu mock `IVnPayService`), lỗi 400 khi lưu tổ hợp (format giờ `HH:mm:ss` vs `HH:mm` strict), khung giờ walk-in hard-code, `<a>` lồng `<a>` ở sidebar, lỗ hổng audit log khi cập nhật tổ hợp.
  - **Staff audit:** Duyệt toàn bộ 14 trang `/elite/*` + `/dashboard/*` bằng tài khoản `staff1@prosport.vn`; sửa `FormatException` do specifier `HH` không hợp lệ với `TimeSpan` (crash trang Lịch thời gian thực + lỗi 500 khi walk-in cho khách có email), khung giờ POS hard-code.
...
- **Author:** Phạm Nguyễn Tiến Đạt
- **Audit Log:** Log #20
- **Purpose:** Bổ sung tính năng Thêm/Xóa sản phẩm cho trang quản lý kho Admin; khắc phục hai lỗi dữ liệu sản phẩm phát sinh trong quá trình kiểm thử tính năng (hardcode Category/SportType/StockQuantity khi tạo mới, sai thứ tự ưu tiên khi gán ảnh sản phẩm); cập nhật ảnh sản phẩm theo yêu cầu.
...
- **Expected Output:** Admin tạo và xóa được sản phẩm với dữ liệu lưu đúng theo lựa chọn thực tế (không còn giá trị hardcode); ảnh sản phẩm hiển thị đúng theo ImageUrl đã gán, không bị ghi đè bởi cơ chế match-từ-khóa; có test tự động khóa lại hành vi đúng cho cả hai lỗi đã sửa; ảnh sản phẩm "Cascade Drive" được cập nhật đúng theo file người dùng cung cấp.
- **Evaluation:** AI xác định đúng phạm vi xử lý khi phát hiện lỗi ngoài yêu cầu ban đầu, truy đến tận nguồn gốc ở tầng service/logic thay vì chỉ che ở giao diện, và bổ sung test tự động cho từng lỗi trước khi xác nhận hoàn thành. Toàn bộ bộ test liên quan (74/74 phía frontend, cùng các test backend liên quan) được chạy lại để đảm bảo không phát sinh hồi quy. Khi nhận ảnh sản phẩm dán trực tiếp vào hội thoại, AI không tự ý sử dụng mà xác nhận rõ giới hạn kỹ thuật (không có đường dẫn file khả dụng) và yêu cầu người dùng lưu ảnh vào máy trước; khi phát hiện file mới, AI đọc lại nội dung để xác nhận đúng ảnh trước khi đưa vào dự án, tránh giả định sai lệch.


---
### Prompt #23
- **Date:** 2026-07-22
- **AI Tool:** Antigravity / Claude Code
- **Author:** Phạm Nguyễn Tiến Đạt
- **Audit Log:** Log #23
- **Purpose:** Đồng bộ dữ liệu hạt giống (seed data) thiết bị vào source code; rà soát và dọn dẹp các rủi ro bảo mật trong file cấu hình; tối ưu hóa giao diện thông báo theo chuẩn Taste Skill (`DESIGN.md`); commit và đẩy code lên nhánh `DE190147/audit-module`.
- **Prompt:** Yêu cầu đồng bộ dữ liệu sản phẩm từ cơ sở dữ liệu lên mã nguồn (DatabaseSeeder) và thay đổi đường dẫn ảnh tĩnh để đảm bảo tính nhất quán khi tải về trên các môi trường khác. Chỉ đạo rà soát toàn diện frontend và cấu hình để phát hiện, khắc phục các bất cập hoặc lỗi bảo mật tiềm ẩn. Áp dụng chuẩn Taste Skill để tối ưu hóa UI, đặc biệt yêu cầu tinh chỉnh hộp thoại thông báo (Notification Box) để loại bỏ thiết kế "robot" khô cứng, mang lại trải nghiệm mượt mà và cao cấp hơn. Cuối cùng, yêu cầu đóng gói toàn bộ thay đổi và đẩy lên nhánh DE190147/audit-module.
- **Expected Output:** Mã nguồn `DatabaseSeeder` được đồng bộ trọn vẹn 47 sản phẩm với đường dẫn ảnh tương đối, không bị lỗi hardcode domain local (`localhost`); Không còn thông tin nhạy cảm (Gmail Password, Hash Secret) bị lộ trong file `appsettings.json`; Giao diện hộp thoại thông báo hiển thị tinh tế (bo tròn mềm mại, đổ bóng nổi, hiệu ứng đổi màu mượt mà theo đúng Taste Skill); Code được commit nguyên vẹn và push thành công lên đúng nhánh làm việc.
- **Evaluation:** AI thể hiện sự nhạy bén trong việc bảo vệ tính an toàn và tính linh hoạt (portability) của dự án. Thay vì chỉ copy dữ liệu một cách thụ động, AI chủ động phát hiện lỗi hardcode URL tĩnh gây hỏng ảnh trên Production và nguy cơ lộ lọt mật khẩu trong cấu hình, từ đó xử lý triệt để trước khi người khác clone project về. Trong tác vụ UI, AI nắm bắt chính xác ý định "smooth" của người dùng, tự động phân tích quy tắc từ file `DESIGN.md` và áp dụng thành công lên component, nâng cấp đáng kể cảm giác cao cấp (Premium) thay vì dùng các style rập khuôn. Cuối cùng, khả năng điều khiển Git thông qua dòng lệnh được thực thi chuẩn xác, trơn tru.
