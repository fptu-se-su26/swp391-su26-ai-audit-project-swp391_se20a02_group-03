...
- **AI Tool:** Cursor (Claude Opus)
- **Author:** Phạm Nguyễn Tiến Đạt
- **Purpose:** Hoàn thiện luồng xác thực Google OAuth end-to-end, làm mới nhận diện thương hiệu PRO-SPORT, rà soát Việt hóa toàn hệ thống và chuẩn hóa cấu hình môi trường phát triển an toàn.
...
- **Expected Output:** Luồng đăng nhập/đăng ký Google hoạt động end-to-end (Frontend `@react-oauth/google` + Backend validate JWT). Component `GoogleSignInButton.jsx`, utility `googleAuth.js`, cập nhật `AuthService.cs` và endpoint `POST /api/auth/google-login`. Logo mới (`ProSportLogoMark.jsx`, `ProSportLogo.jsx`, `public/logo.svg`, favicon) áp dụng đồng bộ trên toàn hệ thống. Việt hóa 80+ trang/component, chuẩn hóa `StatusBadge`/`labels.js`. Script `setup-local.ps1`, file `.example` cho cấu hình dev; loại secret khỏi Git.
- **Evaluation:** AI đã triển khai đúng kiến trúc OAuth end-to-end, sinh component logo và quét Việt hóa quy mô lớn, đồng thời thiết lập cơ chế cấu hình dev an toàn (gitignore + file mẫu). Tuy nhiên, luồng OAuth không thể hoạt động ngay do các lỗi cấu hình môi trường mà AI không thể tự xử lý phía Google Cloud. Tôi đã trực tiếp can thiệp (Human Decision): tạo OAuth Web Client trên GCP, thêm Authorized JavaScript Origins (`http://localhost:5173`, `http://127.0.0.1:5173`) và Test Users — khắc phục lỗi `The given origin is not allowed for the given client ID`; phát hiện và sửa typo 1 ký tự trong Client ID (`...ubquh...` → `...u5quh...`) gây lỗi `client ID is not found`. Về thiết kế logo, tôi đã yêu cầu tinh giản qua nhiều vòng (tối giản, tránh giống thương hiệu đối thủ) trước khi chốt phương án lục giác + sân nhìn từ trên. Cuối cùng, chỉ đạo commit 121 file, `pull --rebase` trước push và loại trừ file tạm khỏi commit; đăng nhập Google xác nhận thành công, push lên nhánh `DE190147/audit-module` (commit `fed44de`).
