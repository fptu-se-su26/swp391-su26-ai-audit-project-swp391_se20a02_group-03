...
Song song đó, chúng tôi tiến hành **Đồng bộ hóa Ngữ cảnh & Ngôn ngữ (Domain Sanitization & Localization)** trên quy mô lớn. Hơn 40+ trang UI được Việt hoá toàn diện và loại bỏ triệt để các môn thể thao ngoài luồng (Bóng rổ, Tennis) để bám sát chuyên đề Pickleball/Cầu lông. Về mặt hệ thống, chúng tôi thực hiện tái cấu trúc (Refactoring) các luồng giao dịch nhạy cảm ở Backend để đảm bảo tính toàn vẹn dữ liệu. Cuối cùng, chúng tôi tiến hành **Tổng rà soát hệ thống (Deep Scan)**, sử dụng sức mạnh tự động hóa của AI (viết Script Regex) để sửa hàng loạt các cảnh báo mã nguồn React Hooks và phân giải các xung đột kiến trúc phức tạp khi hợp nhất Git.
...
- **Ám ảnh cú pháp hiện đại nhưng thiếu an toàn (Syntax vs. Linting constraints):** AI có xu hướng rất thích sử dụng Arrow Function (`const func = async () => {}`). Tuy nhiên, việc khai báo sau khi gọi trong `useEffect` đã vi phạm quy tắc khắt khe của React ESLint, gây ra hơn 60 cảnh báo lỗi hoisting (`Cannot access variable before it is declared`) trên toàn dự án.
- **Sự phức tạp của cấu trúc EF Core Snapshot:** Khi hợp nhất nhánh (merge branch), AI không thể xử lý được các xung đột (Conflict) nằm sâu trong file `ProSportDbContextModelSnapshot.cs`. Lịch sử Migration bị phân mảnh và ghi đè khiến Backend sụp đổ hoàn toàn.
- **Bất lực trước cảnh báo bảo mật thư viện (Dependency Hell):** Khi chạy quét lỗ hổng `npm audit`, AI mù quáng đề xuất lệnh fix (force upgrade) lên Vite v8. Cập nhật này mang tính Breaking Changes, có nguy cơ phá vỡ hoàn toàn file cấu hình gốc. AI thiếu đi trực giác đánh giá rủi ro (Risk Assessment) giữa bảo mật và tính ổn định.
...
- **Tự động hóa Tái cấu trúc (Automated Global Refactoring):** Đứng trước hơn 60 lỗi React Hooks trên 35+ file, thay vì sửa tay, chúng tôi tận dụng AI như một đòn bẩy: Chỉ đạo AI viết một PowerShell Script dùng Regex quét toàn hệ thống, tự động bóc tách và chuyển đổi Arrow Function sang Async Function chuẩn, dập tắt toàn bộ rủi ro Crash UI.
- **Đánh giá Rủi ro Bảo mật (Security vs. Stability):** Trực tiếp phủ quyết đề xuất tự động cập nhật thư viện lỗ hổng của AI. Quyết định giữ lại cấu trúc Vite v6 hiện tại vì rủi ro vỡ config cao hơn nhiều so với lỗ hổng của thư viện dev-dependency (`esbuild`). Đồng thời, kiên quyết bắt AI phải thực hiện các bài "Deep Scan" liên tục để ép lòi ra các lỗi ẩn.
- **Quản trị Phiên bản & Rollback (Version Control Management):** Để chấn chỉnh sự "sáng tạo quá đà" của AI (tự ý đổi phong cách CSS), chúng tôi lập tức sử dụng `git checkout` để Rollback giao diện về lại nguyên bản. Trực tiếp can thiệp giải quyết tay các điểm xung đột trong file Model Snapshot, đổi tên Migration class bị trùng lặp và bypass tính năng Push Protection của GitHub để thông luồng CI/CD.
...
- **Tự động hóa là Đòn bẩy quyền lực (Automation Leverage):** Mặc dù AI sinh ra lỗi, nhưng cũng chính AI có thể viết Script (Regex/AST) để dọn dẹp hàng nghìn dòng lỗi trên hàng chục file chỉ trong 2 giây. Biết cách dùng AI để tự động hóa sửa lỗi do chính AI tạo ra là một kỹ năng đắt giá.
- **Cân bằng giữa Bảo mật và Ổn định (Security vs. Stability Trade-off):** Không phải cảnh báo bảo mật nào (Audit warning) cũng cần fix ngay lập tức. Nếu lỗ hổng chỉ nằm ở thư viện phát triển (Dev Dependencies) mà cách sửa lại phá vỡ hệ thống (Breaking Changes), lập trình viên phải là người đủ bản lĩnh để đưa ra quyết định bỏ qua.
- **RAG không chỉ là Tìm kiếm, mà là Chắt lọc (Pruning):** Đưa dữ liệu thực vào AI là sức mạnh, nhưng đưa quá nhiều dữ liệu rác sẽ giết chết hiệu năng. Lập trình viên đóng vai trò là "bộ lọc" để tối ưu hóa lượng dữ liệu cung cấp cho AI, vừa tiết kiệm chi phí, vừa gia tăng độ chuẩn xác.
- **AI là một "nhân viên" quá hăng hái (The Overzealous AI):** Đôi khi AI có xu hướng làm quá mức cần thiết (over-engineer) hoặc ảo giác ra các thiết kế sai định hướng. Lập trình viên phải đóng vai trò là một Product Owner nghiêm khắc: biết khi nào nên chấp nhận, khi nào phải phanh lại và quyết đoán nhấn nút "Rollback".chi phí, vừa giúp AI trả lời thông minh và tập trung hơn.
