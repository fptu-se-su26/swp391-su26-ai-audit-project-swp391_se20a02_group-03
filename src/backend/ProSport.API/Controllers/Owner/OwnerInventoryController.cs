...
public class OwnerInventoryController : OwnerControllerBase
{
    ...
    [HttpGet("rental-assets")]
    public async Task<IActionResult> GetRentalAssets([FromQuery] OwnerInventoryQueryDto query)
    {
        var userId = GetUserId();
        if (userId == null) return UnauthorizedResult();
        try
        {
            await _ownerAccessService.RequireOwnerOrAdminAccessAsync(userId.Value, query.ComplexId, IsAdmin);
            var response = await _inventoryService.GetRentalAssetsAsync(query);
            return StatusCode(response.StatusCode, response);
        }
        catch (Exception ex) { return FromOwnerException(ex); }
    }

    [HttpPost("rental-assets")]
    public async Task<IActionResult> CreateRentalAsset([FromBody] CreateRentalAssetDto dto)
    {
        var userId = GetUserId();
        if (userId == null) return UnauthorizedResult();
        try
        {
            await _ownerAccessService.RequireOwnerOrAdminAccessAsync(userId.Value, dto.ComplexId, IsAdmin);
            var response = await _inventoryService.CreateRentalAssetAsync(userId.Value, dto);
            return StatusCode(response.StatusCode, response);
        }
        catch (Exception ex) { return FromOwnerException(ex); }
    }

    [HttpPut("rental-assets/{id:int}")]
    public async Task<IActionResult> UpdateRentalAsset(int id, [FromQuery] int complexId, [FromBody] UpdateRentalAssetDto dto)
    {
        var userId = GetUserId();
        if (userId == null) return UnauthorizedResult();
        try
        {
            await _ownerAccessService.RequireOwnerOrAdminAccessAsync(userId.Value, complexId, IsAdmin);
            var response = await _inventoryService.UpdateRentalAssetAsync(userId.Value, id, complexId, dto);
            return StatusCode(response.StatusCode, response);
        }
        catch (Exception ex) { return FromOwnerException(ex); }
    }

    [HttpPatch("rental-assets/{id:int}/status")]
    public async Task<IActionResult> UpdateRentalAssetStatus(int id, [FromQuery] int complexId, [FromBody] UpdateRentalAssetStatusDto dto)
    {
        var userId = GetUserId();
        if (userId == null) return UnauthorizedResult();
        try
        {
            await _ownerAccessService.RequireOwnerOrAdminAccessAsync(userId.Value, complexId, IsAdmin);
            var response = await _inventoryService.UpdateRentalAssetStatusAsync(userId.Value, id, complexId, dto);
            return StatusCode(response.StatusCode, response);
        }
        catch (Exception ex) { return FromOwnerException(ex); }
    }
