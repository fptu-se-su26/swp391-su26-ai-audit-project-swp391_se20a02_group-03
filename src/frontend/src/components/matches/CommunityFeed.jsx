...
import { MapPin, Users, Trophy, Swords, Clock, Calendar, Wallet, Radar, Plus } from 'lucide-react'
...
export default function CommunityFeed() {
  ...
  return (
    <div className="grid grid-cols-[1fr_320px] max-lg:grid-cols-1 gap-8 items-start w-full max-w-[1200px] mx-auto font-sans auth-animate-in">
      ...
      <div className="flex flex-col min-w-0 gap-6">
        ...
        {/* Filters & Nút Tạo Trận (Đã dời từ Sidebar) */}
        <div className="flex max-sm:flex-col sm:items-center justify-between gap-4">
          <div className="flex gap-2 flex-wrap">
            {sportFilters.map(f => (
              <button
                key={f}
                className={`px-5 py-2.5 rounded-full text-[13.5px] font-bold transition-all duration-300 cursor-pointer outline-none border-0 ${
                  activeFilter === f
                    ? 'bg-[#0f172a] text-white shadow-[0_4px_12px_rgba(15,23,42,0.2)]'
                    : 'bg-white text-gray-500 hover:bg-gray-50 hover:text-gray-900 shadow-sm border border-gray-100'
                }`}
          ...
          <Link 
            to="/matches/create" 
            className="flex items-center justify-center gap-2 bg-[#14b8a6] hover:bg-[#15c3b0] text-white py-2.5 px-6 rounded-full text-[13px] font-bold uppercase tracking-wide transition-all duration-300 shadow-[0_4px_12px_rgba(20,184,166,0.3)] hover:shadow-[0_6px_16px_rgba(20,184,166,0.4)] hover:-translate-y-0.5 no-underline shrink-0"
          >
             ...
             return (
              <div key={m.matchId} className={`p-4 flex flex-col gap-2.5 transition-all duration-300 ${isExpired ? 'opacity-60 grayscale-[40%]' : 'hover:shadow-[0_8px_24px_-4px_rgba(0,0,0,0.08)] hover:-translate-y-0.5 cursor-pointer'} ${modernCardClass}`}>
                ...
                <div className="flex items-center justify-between mt-1 pt-3 border-t border-gray-100">
                  ...
                  {isExpired ? (
                    <button disabled className="bg-gray-200 text-gray-500 px-5 py-2 rounded-[8px] text-[13px] font-bold uppercase tracking-wide cursor-not-allowed border-0">
                      Đã kết thúc
                    </button>
                  ) : (
      ...
      <div className="flex flex-col gap-6 max-lg:col-span-full max-lg:grid max-lg:grid-cols-2 max-md:grid-cols-1 shrink-0 sticky top-0">
