...
export default function AdminKycPage() {
  ...
  async function reject() {
    ...
    try {
      setActing(true)
      const res = await kycApi.reject(selected.ekycProfileId, reason.trim())
      if (res.statusCode === 200) { addToast('Đã từ chối hồ sơ.', 'success'); load() }
      else addToast(res.message || 'Từ chối thất bại.', 'error')
    } catch (err) {
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-slate-500">Kiểm tra giấy tờ tùy thân để mở khóa Ví Escrow và quyền Host kèo.</p>
        </div>
              ...
              <>
                ...
                <div className="p-6 flex-1 space-y-6">
                  ...
                  <div className="bg-slate-50 rounded-lg p-4 border border-slate-100 grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <p className="text-[0.65rem] text-slate-500 uppercase tracking-wider mb-1">Email</p>
                  ...
                  {selected.status === 'Pending' && (
                    <div className="flex gap-3 pt-4 border-t border-slate-100">
                      <button onClick={reject} disabled={acting} className="flex-1 border border-red-200 text-red-500 font-bold py-2.5 rounded-lg hover:bg-red-50 transition-colors disabled:opacity-60">Từ chối</button>
