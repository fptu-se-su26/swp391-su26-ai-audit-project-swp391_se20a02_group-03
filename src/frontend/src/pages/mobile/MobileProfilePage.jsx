...
const MENU_ITEMS = [
  { label: 'Thông tin tài khoản', icon: 'user', to: '/apex/profile' },
  { label: 'Phương thức thanh toán', icon: 'card', to: '/apex/settings' },
  { label: 'Thông báo', icon: 'bell', to: '/apex/settings' },
]
        ...
        <div className="pt-6 px-4 text-center flex flex-col items-center">
          ...
          {!user ? (
            ...
            <div className="flex gap-3 mt-5 w-full">
              <Link
                to="/apex/profile"
                className="flex-1 bg-slate-900 text-[var(--theme-primary)] text-sm font-semibold py-3 rounded-xl transition-all active:scale-[0.98] text-center no-underline"
              >
                Chỉnh sửa hồ sơ
              </Link>
              <Link
                to="/apex/settings"
                className="w-12 h-12 flex items-center justify-center bg-white border border-slate-200 text-slate-700 rounded-xl transition-all active:bg-slate-50"
                aria-label="Cài đặt"
              >
        ...
        <div className="px-4 mb-8">
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden divide-y divide-slate-100">
            {MENU_ITEMS.map((item, i) => (
              <Link key={item.label} to={item.to} className="flex items-center gap-4 p-4 hover:bg-slate-50 transition-colors group no-underline">
