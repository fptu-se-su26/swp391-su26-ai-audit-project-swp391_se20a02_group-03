...
export default function ShopCheckoutPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f5f9fc]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_380px] gap-7 max-w-[1100px] my-8 mx-auto px-6 flex-1 items-start w-full">
        <div className="flex flex-col gap-5">
          ...
          <div className="flex items-center mb-2">
            {steps.map((s, i) => (
              <div key={s} className={`flex items-center gap-2 flex-1 ${i <= step ? 'text-[#14B8A6]' : ''}`}>
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${i <= step ? 'bg-[#14B8A6] text-[var(--theme-primary)]' : 'bg-[#e0ecf0] text-[#94a3b8]'}`}>{i + 1}</div>
                <span className={`text-xs font-semibold ${i <= step ? 'text-[#14B8A6]' : 'text-[#94a3b8]'}`}>{s}</span>
                {i < steps.length - 1 && <div className={`flex-1 h-[1.5px] mx-2 ${i < step ? 'bg-[#14B8A6]' : 'bg-[#e0ecf0]'}`} />}
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-[14px] p-6 border-[1.5px] border-[#e0ecf0]">
            ...
            <div className="grid grid-cols-2 gap-3.5">
              <div className="flex flex-col gap-1.5">
                ...
                <input id="s-fname" type="text" defaultValue="John" className="border-[1.5px] border-[#e0ecf0] rounded-lg px-3 py-2.5 font-sans text-sm text-foreground outline-none transition-colors focus:border-[#14B8A6]" />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-[0.78rem] font-semibold text-[#94a3b8]" htmlFor="s-lname">Last Name</label>
                <input id="s-lname" type="text" defaultValue="Doe" className="border-[1.5px] border-[#e0ecf0] rounded-lg px-3 py-2.5 font-sans text-sm text-foreground outline-none transition-colors focus:border-[#14B8A6]" />
              </div>
              <div className="flex flex-col gap-1.5 col-span-2">
                <label className="text-[0.78rem] font-semibold text-[#94a3b8]" htmlFor="s-email">Email Address</label>
                <input id="s-email" type="email" defaultValue="john.doe@example.com" className="border-[1.5px] border-[#e0ecf0] rounded-lg px-3 py-2.5 font-sans text-sm text-foreground outline-none transition-colors focus:border-[#14B8A6]" />
              </div>
              <div className="flex flex-col gap-1.5 col-span-2">
                <label className="text-[0.78rem] font-semibold text-[#94a3b8]" htmlFor="s-address">Street Address</label>
                <input id="s-address" type="text" defaultValue="123 Performance Way" className="border-[1.5px] border-[#e0ecf0] rounded-lg px-3 py-2.5 font-sans text-sm text-foreground outline-none transition-colors focus:border-[#14B8A6]" />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-[0.78rem] font-semibold text-[#94a3b8]" htmlFor="s-city">City</label>
                <input id="s-city" type="text" defaultValue="Athletica" className="border-[1.5px] border-[#e0ecf0] rounded-lg px-3 py-2.5 font-sans text-sm text-foreground outline-none transition-colors focus:border-[#14B8A6]" />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-[0.78rem] font-semibold text-[#94a3b8]" htmlFor="s-zip">Postal Code</label>
                <input id="s-zip" type="text" defaultValue="10000" className="border-[1.5px] border-[#e0ecf0] rounded-lg px-3 py-2.5 font-sans text-sm text-foreground outline-none transition-colors focus:border-[#14B8A6]" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[14px] p-6 border-[1.5px] border-[#e0ecf0]">
            ...
            {[{id:'express',label:'Express Delivery',sub:'1-2 Business Days',price:'$15.00'},{id:'standard',label:'Standard Delivery',sub:'3-5 Business Days',price:'Free'}].map(d => (
              <label key={d.id} className={`flex items-center gap-3.5 border-[1.5px] rounded-lg p-3.5 px-4 cursor-pointer mb-2 transition-all duration-200 ${delivery === d.id ? 'border-[#14B8A6] bg-[#14B8A6]/[0.04]' : 'border-[#e0ecf0]'}`} htmlFor={`del-${d.id}`}>
                ...
                <span className="ml-auto text-sm font-bold text-foreground">{d.price}</span>
              </label>
            ))}
          </div>
          ...
          <div className="bg-white rounded-[14px] p-6 border-[1.5px] border-[#e0ecf0]">
            ...
            <div className="flex gap-2.5 mb-4 flex-wrap">
              {payMethods.map(m => (
                <button key={m} className={`flex flex-col items-center gap-1.5 border-[1.5px] rounded-lg p-3 px-4 bg-white cursor-pointer font-sans text-[0.78rem] font-semibold transition-all duration-200 min-w-[72px] ${payMethod === m ? 'border-[#14B8A6] text-[#14B8A6] bg-[#14B8A6]/5' : 'border-[#e0ecf0] text-[#64748b] hover:border-[#14B8A6] hover:text-[#14B8A6]'}`} onClick={() => setPayMethod(m)}>
              ...
              <div className="border-[1.5px] border-[#e0ecf0] rounded-lg p-4 flex flex-col gap-3.5">
                <div className="flex flex-col gap-1.5 col-span-2">
                  ...
                  <div className="flex items-center gap-2 border-[1.5px] border-[#e0ecf0] rounded-lg px-3 py-2.5">
                    ...
                    <input id="card-num" type="text" placeholder="0000 0000 0000 0000" className="border-none outline-none flex-1 font-sans text-sm text-foreground" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3.5">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[0.78rem] font-semibold text-[#94a3b8]" htmlFor="expiry">Expiry Date</label>
                    <input id="expiry" type="text" placeholder="MM/YY" className="border-[1.5px] border-[#e0ecf0] rounded-lg px-3 py-2.5 font-sans text-sm text-foreground outline-none transition-colors focus:border-[#14B8A6]" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[0.78rem] font-semibold text-[#94a3b8]" htmlFor="cvc">CVC</label>
                    <input id="cvc" type="text" placeholder="123" className="border-[1.5px] border-[#e0ecf0] rounded-lg px-3 py-2.5 font-sans text-sm text-foreground outline-none transition-colors focus:border-[#14B8A6]" />
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="bg-white rounded-[14px] p-6 border-[1.5px] border-[#e0ecf0] md:sticky md:top-6">
          ...
          {[{name:'Aero-Dash Elite 2.0',meta:'Size: 10 | Color: Neon',price:'$185.00',img:'https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=100&q=80'},{name:'Compression Pro Tights',meta:'Size: M | Color: Obsidian',price:'$75.00',img:'https://images.unsplash.com/photo-1506629082955-511b1aa562c8?w=100&q=80'}].map(item => (
            <div key={item.name} className="flex items-center gap-3 py-2.5 border-b border-[#f0f5f9]">
              ...
              <div>
                <p className="text-[0.85rem] font-bold text-foreground">{item.name}</p>
                <p className="text-[0.75rem] text-[#94a3b8]">{item.meta}</p>
              </div>
              <span className="ml-auto text-[0.85rem] font-semibold text-foreground whitespace-nowrap">{item.price}</span>
            </div>
          ))}
          ...
          <div className="flex justify-between text-base font-bold text-foreground my-3.5 pt-3 border-t-[1.5px] border-[#e0ecf0]">
            <span>Tổng cộng</span>
            <span className="text-[#14B8A6] text-[1.15rem]">$297.00</span>
          </div>
          <button className="bg-[#14B8A6] hover:bg-[#0b7373] text-[var(--theme-primary)] font-semibold flex items-center justify-center w-full gap-2 p-3.5 rounded-full text-[0.95rem] border-none cursor-pointer transition-colors">
      ...
      <footer className="bg-[#0d1a24] text-[var(--theme-primary)]/55 flex flex-col md:flex-row items-center justify-between p-4.5 px-10 text-xs gap-5">
        <span className="font-oswald text-base font-bold text-[var(--theme-primary)]">PRO-SPORT</span>
        <p>© 2024 PRO-SPORT. Engineered for Elite Performance.</p>
        <div>
          <a href="#" className="text-[var(--theme-primary)]/55 no-underline ml-3 hover:text-[#0fc8b5]">Privacy</a>
          <a href="#" className="text-[var(--theme-primary)]/55 no-underline ml-3 hover:text-[#0fc8b5]">Terms</a>
        </div>
      </footer>
    </div>
  )
